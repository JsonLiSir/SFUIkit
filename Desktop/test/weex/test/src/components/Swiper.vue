<template>
    <section class="wh_content" @touchmove="fn">
        ...
        <div v-if="showIndicator" class="wh_indicator">
            <div v-for="(tag,$index) in slidesLength" :key="$index" v-bind:class="{ wh_show_bgcolor: index-1==$index || (slidesLength+1 == index && $index==0) || (slidesLength-1 == $index && index==0)}" class="wh_indicator_item">
            </div>
        </div>
    </section>
</template>
    ...
    export default {
        ...
        mounted() {
            ...
            setTimeout(() => {
                // 克隆dom
                this.starDom()
                this.dom.transform = `translate3d(${this.offsetWidth * -1}px, 0px, 0px)`
                this.dom['-webkit-transform'] = `translate3d(${this.offsetWidth * -1}px, 0px, 0px)`
                this.dom['-ms-transform'] = `translate3d(${this.offsetWidth * -1}px, 0px, 0px)`
                if (this.autoPlay) {
                    this.setTime()
                }
            }, 50)
        ...
        methods: {
            ...
            e(x) {
                if (this.slideing && this.t.s !== -1) {
                    ...
                    this.setTransform(this.t.m + this.t.sx)
                    x = this.getTransform()
                    x += this.t.m > 0 ? this.offsetWidth * 0.3 : this.offsetWidth * -0.3
                    this.index = Math.round(x / this.offsetWidth) * -1
                    this.wh('touch')
                }
            },
            setTransform(num) {
            ...
            wh(type) {
                ...
                this.setTransform(this.index * -1 * this.offsetWidth)
                this.t.m = 0
                this.t.s = -1 // 保证下次重新赋值
                if (this.autoPlay) {
                    this.setTime()
            ...
            starDom() {
                ...
                if (this.slidesLength > 1) {
                    ...
                    this.offsetWidth = document.querySelector('.' + this.className).offsetWidth
