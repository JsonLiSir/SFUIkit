...
#define SFDButtonStateKey(state) [NSString stringWithFormat:@"SFDButton_State_%lu", (unsigned long)state]
...
@property (nonatomic, strong) NSMutableDictionary *sf_backgroundColorDic;

@end

@implementation SFDButton
...
- (void)layoutSubviews {
    ...
    [self sf_refreshItemEdgeInsets];
}

- (void)sf_refreshItemEdgeInsets {
    // 借鉴自 QMUIKIT
    ...
    CGFloat spacingBetweenImageAndTitle = isImageViewShowing && isTitleLabelShowing ? self.sf_imageTitleSpace : 0;// 如果图片或文字某一者没显示，则这个 spacing 不考虑进布局
    ...
    if (self.sf_imageLocation== SFDButtonImageLocationTop || self.sf_imageLocation== SFDButtonImageLocationBottom) {
        ...
        if (self.sf_imageLocation== SFDButtonImageLocationTop) {
    ...
    } else if (self.sf_imageLocation== SFDButtonImageLocationLeft || self.sf_imageLocation== SFDButtonImageLocationRight) {
        ...
        if (self.sf_imageLocation== SFDButtonImageLocationLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.sf_imageLocation) {
        case SFDButtonImageLocationTop:
        case SFDButtonImageLocationBottom: {
        ...
        case SFDButtonImageLocationLeft:
        case SFDButtonImageLocationRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 SFDButtonImageLocationLeft 并且titleLabel 多行的情况下，QMUIButton 计算的结果与系统不一致
...
- (void)setsf_imageLocation:(SFDButtonImageLocation)sf_imageLocation {
    if (sf_imageLocation != _sf_imageLocation) {
        _sf_imageLocation = sf_imageLocation;
        [self setNeedsLayout];
    }
}

- (void)setsf_imageTitleSpace:(CGFloat)sf_imageTitleSpace {
    if (sf_imageTitleSpace != _sf_imageTitleSpace) {
        _sf_imageTitleSpace = sf_imageTitleSpace;
        [self setNeedsLayout];
    }
}


- (NSMutableDictionary *)sf_backgroundColorDic {
    if (!_sf_backgroundColorDic) {
        _sf_backgroundColorDic = [NSMutableDictionary new];
    }
    return _sf_backgroundColorDic;
}

- (void)setBackgroundColor:(UIColor *)backgroundColor {
    [self sf_setBackgroundColor:backgroundColor forState:UIControlStateNormal];
}

- (void)sf_setBackgroundColor:(UIColor *)color forState:(UIControlState)state {
    [self.sf_backgroundColorDic setObject:color forKey:SFDButtonStateKey(state)];
    if (self.state == state) {
...
- (UIColor *)sf_backgroundColorForState:(UIControlState)state {
    return _sf_backgroundColorDic[SFDButtonStateKey(state)];
}

- (void)setSelected:(BOOL)selected {
    [super setSelected:selected];
    [self sf_updateBackgroundColor];
}
...
- (void)sf_updateBackgroundColor {
    UIColor *color = _sf_backgroundColorDic[SFDButtonStateKey(self.state)];
    if (!color) {
        color = _sf_backgroundColorDic[SFDButtonStateKey(UIControlStateNormal)];
    }
